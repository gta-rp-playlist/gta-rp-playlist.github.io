    ...
    toggleSidebarButton.addEventListener('click', () => {
        ...
        players.slice(3,7).forEach((player, index) => {
            ...
            if (numPlayers === 10 && isHidden) {
                ...
                const player1 = document.getElementById('twitch-player-1');
                const players = Array.from({ length: 10 }, (_, i) => document.getElementById(`twitch-player-${i + 1}`));
            ...
            if (numPlayers === 11 && isHidden) {
                // Set up Player 1 separately
                const player1 = document.getElementById('twitch-player-1');
                const players = Array.from({ length: 11 }, (_, i) => document.getElementById(`twitch-player-${i + 1}`));
            ...
            if (numPlayers === 12 && isHidden) {
                // Set up Player 1 separately
                const player1 = document.getElementById('twitch-player-1');
                const players = Array.from({ length: 12 }, (_, i) => document.getElementById(`twitch-player-${i + 1}`));
            ...
            if (numPlayers === 13 && isHidden) {
                // Set up Player 1 separately
                const player1 = document.getElementById('twitch-player-1');
                const players = Array.from({ length: 13 }, (_, i) => document.getElementById(`twitch-player-${i + 1}`));
            ...
            if (numPlayers >= 14 && isHidden) {
                // placeholder
                const players = Array.from({ length: numPlayers }, (_, i) => document.getElementById(`twitch-player-${i + 1}`));
                const viewportWidth = window.innerWidth;
                const viewportHeight = window.innerHeight;
...
function loadSingleTwitchPlayer(username, index) {
    ...
    // Retrieve the player title from the sidebar
    const sidebarItem = Array.from(document.querySelectorAll('#sidebar .sub-item')).find(item => {
    ...
    // Check if the title contains the specific phrase for Kick player
    if (title.includes("🟢🟢Kick Stream☝️") || Fieldbox === "kick") {
        kickPlayerUsername = username; // Set the username for the Kick player

        // Create Kick iframe
        const kickIframe = document.createElement('iframe');

        // Determine if the Kick player should be muted or unmuted
        if (playerDiv.id !== 'twitch-player-1') {
            kickIframe.src = `https://player.kick.com/${username}?muted=true`;
            isKickMuted = true; // Track as muted
            playerDiv.setAttribute('data-volume', 'playermuted'); // Set volume attribute for muted state
        } else {
            kickIframe.src = `https://player.kick.com/${username}?muted=false`;
            isKickMuted = false; // Track as unmuted
            playerDiv.setAttribute('data-volume', 'playerunmuted'); // Set volume attribute for unmuted state
        }
                ...
                if (isKickMuted) {
                    ...
                    players.forEach(otherPlayer => {
                        if (otherPlayer.username !== username) {
                            ...
                            if (otherPlayer.playerDiv) { // Check if playerDiv exists
                                otherPlayer.playerDiv.setAttribute('data-volume', 'playermuted'); // Update data-volume attribute
                            }
                            console.log(`Volume set to 0.0 for player: ${otherPlayer.username}`);
                        }
                    });

                    // Recreate the Kick player as unmuted
                    console.log(`Recreating Kick player for ${kickPlayerUsername} unmuted.`);

                    const newKickIframe = document.createElement('iframe');
                    newKickIframe.src = `https://player.kick.com/${kickPlayerUsername}?muted=false`;
                    newKickIframe.style.width = '100%';
                    newKickIframe.style.height = '100%';
                    newKickIframe.frameBorder = '0';
                    newKickIframe.scrolling = 'no';
                    newKickIframe.allowFullscreen = true;

                    playerDiv.setAttribute('data-platform', 'kick');
                    playerDiv.innerHTML = ''; // Clear the old content
                    playerDiv.appendChild(newKickIframe); // Add the new iframe
                    isKickMuted = false; // Update the mute state
                    playerDiv.setAttribute('data-volume', 'playerunmuted'); // Update data-volume for unmuted state

                    console.log(`Recreated Kick player for ${kickPlayerUsername} unmuted. isKickMuted: ${isKickMuted}`);
                } else {
                    console.log(`Kick player for ${kickPlayerUsername} is already unmuted, no need to recreate.`);
                }
            }, 1100);
        });
        ...
        player.username = username; // Add username property to the player object
        player.playerDiv = playerDiv; // Add the playerDiv reference to the player object
        players.push(player); // Add player to the array

        // Set the same data attribute for Twitch players
        playerDiv.setAttribute('data-platform', 'twitch');

        // Set data-volume attribute for Twitch players
        if (index === 1) {
            playerDiv.setAttribute('data-volume', 'playerunmuted'); // First player
        } else {
            playerDiv.setAttribute('data-volume', 'playermuted'); // Subsequent players
        }
        ...
        playerDiv.addEventListener('mouseenter', () => {
            clearTimeout(hoverOutTimer); // Clear any existing timer

            hoverTimer = setTimeout(() => {
                // Check if a Kick player is present and its muted state
                const kickPlayerDiv = Array.from(document.querySelectorAll('.player')).find(playerDiv => 
                    playerDiv.getAttribute('data-platform') === 'kick'
                );

                if (kickPlayerDiv) {
                    const kickIframe = kickPlayerDiv.querySelector('iframe');
                    const kickPlayerMuted = kickIframe.src.includes('muted=true'); // Check if muted

                    // Log the Kick muted state
                    console.log(`isKickMuted: ${kickPlayerMuted}`);

                    console.log(`A Kick player is currently present. It is ${kickPlayerMuted ? 'muted' : 'unmuted'}.`);

                    // If the Kick player is unmuted, recreate the iframe as muted
                    if (!kickPlayerMuted) {
                        console.log(`Recreating Kick player for ${kickPlayerUsername} as muted.`);

                        // Create a new muted Kick iframe
                        const newKickIframe = document.createElement('iframe');
                        newKickIframe.src = `https://player.kick.com/${kickPlayerUsername}?muted=true`;
                        newKickIframe.style.width = '100%';
                        newKickIframe.style.height = '100%';
                        newKickIframe.frameBorder = '0';
                        newKickIframe.scrolling = 'no';
                        ...
                        // Clear the old content and append the new iframe
                        kickPlayerDiv.innerHTML = ''; // Clear old content
                        kickPlayerDiv.appendChild(newKickIframe); // Add the new iframe
                        isKickMuted = true; // Update the mute state
                        kickPlayerDiv.setAttribute('data-volume', 'playermuted'); // Update data-volume for muted state

                        // Log the updated state of isKickMuted
                        console.log(`Recreated Kick player for ${kickPlayerUsername} as muted. isKickMuted: ${isKickMuted}`);
                    }
                } else {
                    console.log("No Kick player is currently present.");
                }
                ...
                players.forEach(otherPlayer => {
                    if (otherPlayer !== player) {
                        otherPlayer.setVolume(0.0);
                        otherPlayer.playerDiv.setAttribute('data-volume', 'playermuted'); // Update data-volume attribute
                        console.log(`Volume set to 0.0 for player: ${otherPlayer.username}`); // Correctly reference username
                    }
                });

                player.setVolume(1.0);
                playerDiv.setAttribute('data-volume', 'playerunmuted'); // Update data-volume for unmuted state
                console.log(`Volume set to 1.0 for player: ${username}`); // Correctly reference username
            }, 1100);
        });
        ...
        playerDiv.addEventListener('mouseleave', () => {
            ...
            // Start a new timer that waits for 3 seconds before resetting the volume
            hoverOutTimer = setTimeout(() => {
                // Add any functionality if needed when the mouse leaves
            });
        });
    }
}
